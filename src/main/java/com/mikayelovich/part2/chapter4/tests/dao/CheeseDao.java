package com.mikayelovich.part2.chapter4.tests.dao;
...
public class CheeseDao {
    ...
    public Cheese getById(Integer id) {
        System.out.println("Loading Cheese from DAO. ID: " + id);

        if (id >= 0 && id < 12) {
            return cheeseList.get(id);
        }
        throw new RuntimeException("Cheese with id" + id + " not found.");
    }
}
